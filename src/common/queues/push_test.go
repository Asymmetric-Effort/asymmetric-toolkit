...
func TestFifo_Push(t *testing.T) {
	...
	errors.Assert(q.Length() == 2,
		fmt.Sprintf("Step 1. Queue length expects 2 (capacity). Encountered:%d", q.Length()))
	<-time.After(2 * time.Second)

	t.Logf("Step 2 Length:%d\n", q.Length())
	errors.Assert(q.Length() == 2,
		fmt.Sprintf("Step 2. Queue length expects 2. Encountered:%d", q.Length()))
	errors.Assert(q.Pop() == "1", "Queue Pop returned the wrong value")

	t.Logf("Step 3 Length:%d\n", q.Length())
	errors.Assert(q.Length() == 2,
		fmt.Sprintf("Step 3. Queue length expects 2. Encountered:%d", q.Length()))
	errors.Assert(q.Pop() == "2", "Queue Pop returned the wrong value")

	t.Logf("Step 4 Length:%d\n", q.Length())
	errors.Assert(q.Length() == 1,
		fmt.Sprintf("Step 4. Queue length expects 1. Encountered:%d", q.Length()))
	errors.Assert(q.Pop() == "3", "Queue Pop returned the wrong value")

	t.Logf("Step 5 Length:%d\n", q.Length())
	errors.Assert(q.Length() == 0,
		fmt.Sprintf("Step 5. Queue length expects 0. Encountered:%d", q.Length()))
