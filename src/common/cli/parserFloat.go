...
/*
	ParserFloat() is a parser function factory which returns a parser function to the caller.
...
func ParserFloat(p ...float64) (parser func(arg *string) (err error, val *Argument)) {
	/*
	...
	*/
	const minimum = 0 // Index for p ...float64 (minimum bounds)
	const maximum = 1 // Index for p ...float64 (maximum bounds)

	getLimit:=func(i int, defaultValue float64) float64{
		//
	...
	func(){
		...
		//
		min:=getLimit(minimum,math.SmallestNonzeroFloat64)
		max:=getLimit(maximum, math.MaxFloat64)
		if min > max {
	...
	parser = func(arg *string) (err error, val *Argument) {
		...
		//
		var value float64 // Our converted value.
		//
		...
		if (value >= min) && (value <= max) {
			...
			return fmt.Errorf("Bounds check fail.  " +
				"Value (%f) falls outside the bound (%f...%f)", value, min, max), nil
		}
	}
