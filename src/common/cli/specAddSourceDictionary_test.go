	...
	func() {
		...
		errors.Assert(o.Argument[sourceDictionaryArgLong].FlagId == FlagSourceDictionary,
			fmt.Sprintf("(%s) expected (%d)  FlagId:%d",
				sourceDictionaryArgLong,
				FlagSourceDictionary,
				o.Argument[sourceDictionaryArgLong].FlagId))

		errors.Assert(o.Argument[sourceDictionaryArgLong].Type == String,
			fmt.Sprintf("String Argument type expected.  Type:%d",
				o.Argument[sourceDictionaryArgLong].Type))

		errors.Assert(o.Argument[sourceDictionaryArgLong].Default == testDefault,
			fmt.Sprintf("Default should be empty for %s",
				sourceDictionaryArgLong))
		...
		errors.Assert(o.Argument[sourceDictionaryArgLong].Parse != nil,
			"Expect non-nil function pointer")
		...
		if o.Argument[sourceDictionaryArgLong].Default != testDefault {
			panic(fmt.Sprintf("Default should be false for domain flag (%d)",
				FlagSourceDictionary))
		}
	}()
}
