...
func TestLogger_WarningStr(t *testing.T) {
	var log Logger
	...
	fmt.Println("------------")
	fmt.Println("")
	for _, msg := range []string{"test1", "test2", "test3"} {
		for _, test := range tests {
			...
			if test.ExpectPrintMsg {
				...
				if event.Message == "" {
					event.Message = fmt.Sprintf("%s", msg)
				}

				errors.Assert(event.EventId == EventStd, fmt.Sprintf("Expected '%d'. "+
					"Encountered: '%d'", Debug, event.EventId))

				errors.Assert(event.Level <= Debug, "Expected Debug level.")

				fmt.Printf("\nEvent message: '%v'\n", event.Message)

				errors.Assert(event.Message == fmt.Sprintf("%s", msg), event.Message)
